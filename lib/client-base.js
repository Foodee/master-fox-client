...
function extractJRObject(data) {
  ...
  var registry = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : {};
  ...
  var relationships = { one: {}, many: {} };
  var ret = new resourceKlass(id, attributes, relationships.one, relationships.many);
  ...
  Object.entries(data.relationships || {}).forEach(function (_ref) {
    ...
    if (relationshipData) {
      ...
      if (Array.isArray(relationshipData)) {
        relationships.many[key] = relationshipData.map(function (nestedRelationshipData) {
          return extractJRObject(findRelationship(included, nestedRelationshipData), included, registry);
        });
      } else {
